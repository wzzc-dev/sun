...
- Renderer/`RendererResources` 支持 cached raw RGBA bytes image/source-rect/sampled/quality draw path，让已解码或外部提供的像素资源复用 image cache、atlas 子图和缩放绘制入口
...
- Renderer/`RendererResources` 支持 cached raw RGBA bytes image/source-rect/sampled/quality draw path，让无容器像素资源可直接进入 image cache、atlas 子图与缩放绘制入口
...
- [x] 将 cached Pixmap/BMP transform-aware sampled/quality draw path 接入 Renderer/`RendererResources`，覆盖 transformed image cache hit/decode telemetry
...
- [x] 将 cached raw RGBA bytes full/source-rect/sampled/quality draw path 接入 Renderer/`RendererResources`，覆盖无容器像素资源的 cache hit/decode telemetry、atlas 子图复用与缩放绘制
