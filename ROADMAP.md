...
- Renderer/`RendererResources` 支持 cached raw RGBA bytes image draw path，让已解码或外部提供的像素资源复用 image cache 和绘制入口
...
- Renderer/`RendererResources` 支持 cached raw RGBA bytes image draw path，让无容器像素资源可直接进入 image cache 与 Canvas 绘制入口
