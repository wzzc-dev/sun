...
- Prefer `render_text_mask_face` and `Renderer::draw_text_face` when connecting
  ...
  `Renderer::draw_text_fallback_line_atlas` when fallback spans should reuse
  ...
  font-face cache reuse plus direct span/text-mask telemetry before cached
  fallback span rendering. Prefer
  ...
  instead of whole rendered text masks while preserving direct span/glyph/atlas
  telemetry. Prefer
