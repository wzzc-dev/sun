...
- `Surface` trait, `MemorySurface`, full/rect present helpers, and packed present batches for `Canvas` and `Pixmap`
...
- `RenderFrame -> NativeSurface` dirty/full present helpers plus adapters for graphics-core strategy-aware submit/query/plan results with state, planned rect/cost, and full-present savings telemetry for window integration
